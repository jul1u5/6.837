...
bool parseFile(istream &in, vector<vector<Vector3f>> *ctrlPoints,
               vector<Curve> *curves, vector<string> *curveNames,
               vector<Surface> *surfaces, vector<string> *surfaceNames) {
    ctrlPoints->clear();
    curves->clear();
    curveNames->clear();
    surfaces->clear();
    ...
    while (in >> objType) {
        ...
        if (objType == "bez2") {
            ...
            // Make the surface
            surfaces->push_back(makeSurfRev(curves->at(it->second), steps));
            ...
            // Make the surface
            surfaces->push_back(
                makeGenCyl(curves->at(itP->second), curves->at(itS->second)));
            surfaceNames->push_back(objName);
            if (named)
                surfaceIndex[objName] = surfaceNames->size() - 1;
