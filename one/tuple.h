...
template <typename TYPE, unsigned SIZE> class tuple {
  public:
    ...
    tuple(const TYPE array[SIZE]) { memcpy(data, array, SIZE * sizeof(TYPE)); }
    ...
    inline TYPE &operator[](unsigned i) { return data[i]; }

    inline const TYPE &operator[](unsigned i) const { return data[i]; }
